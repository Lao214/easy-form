<template>
    <div style="margin:0;">
        <el-row>
            <el-col :span="4">
                <leftBody></leftBody>
            </el-col>
            <el-col :span="20">
                <div
                    style="height: 100vh;background-color: #a7ce74;padding: 1rem;overflow-y: scroll;box-sizing: border-box;">
                    <div style="display: flex; flex-wrap: wrap; padding:0px 70px;">
                        <span
                            style="width: 100%;font-size: 24px;font-weight: 600;text-align: start;margin:20px 10px;"><i
                                class="fa fa-share-alt-square"
                                style="color: rgb(46, 84, 9);font-size: 28px;margin-right: 10px;"></i>分享你的表单</span>
                        <div
                            style="width: 50%;min-height: 392px;height: auto;background: white; box-shadow: 2px 4px 4px rgba(0, 0, 0, 0.429);border-radius: 4px;padding: 20px;">
                            <div style="width: 100%;height: 70px;text-align: start;">
                                <span style="font-weight: 600;color: #49760f;margin-left: 11px;">分享链接</span>
                                <div class="group">
                                    <svg t="1694748450115" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="4042" width="64" height="64">
                                        <path
                                            d="M357.416807 548.994566l306.228473 0 0-76.54535-306.228473 0L357.416807 548.994566zM200.461079 510.692215c0-88.038102 68.861344-156.953681 156.955728-156.953681l114.842585 0 0-72.729441-114.842585 0c-126.337383 0-229.6821 103.34574-229.6821 229.683123 0 126.335337 103.344716 229.683123 229.6821 229.683123l114.842585 0 0-72.728418-114.842585 0C269.322423 667.647943 200.461079 598.678128 200.461079 510.692215M663.64528 281.009092l-114.842585 0 0 72.729441 114.842585 0c88.034009 0 156.898423 68.915579 156.898423 156.953681 0 87.98489-68.863391 156.954705-156.898423 156.954705l-114.842585 0 0 72.728418 114.842585 0c126.33636 0 229.683123-103.347786 229.683123-229.683123C893.32738 384.354831 789.98164 281.009092 663.64528 281.009092"
                                            p-id="4043" fill="#49760f"></path>
                                    </svg>
                                    <!-- <svg class="icon" aria-hidden="true" viewBox="0 0 24 24"><g><path d="M21.53 20.47l-3.66-3.66C19.195 15.24 20 13.214 20 11c0-4.97-4.03-9-9-9s-9 4.03-9 9 4.03 9 9 9c2.215 0 4.24-.804 5.808-2.13l3.66 3.66c.147.146.34.22.53.22s.385-.073.53-.22c.295-.293.295-.767.002-1.06zM3.5 11c0-4.135 3.365-7.5 7.5-7.5s7.5 3.365 7.5 7.5-3.365 7.5-7.5 7.5-7.5-3.365-7.5-7.5z"></path></g></svg> -->
                                    <input v-model="linkDataOnLine" placeholder="link" type="search" class="input"
                                        disabled>
                                </div>
                                <div style="width: 100%;margin-top: 10px;text-align: end;">
                                    <el-button @click="copyLink" type="success"><i class="fa fa-copy"></i>
                                        复制</el-button>
                                    <el-button @click="openInNewTab" type="primary"><i class="fa fa-openid"></i>
                                        在新的页面打开链接</el-button>
                                    <!-- <el-button type="success"><i class="fa fa-qrcode"></i> 下载二维码</el-button> -->
                                </div>
                                <div style="width: 100%;margin-top: 10px;text-align: end;">
                                    <img :src="QRImgUrl" style="width: 192px;height: 192px;">
                                </div>
                            </div>
                        </div>

                        <div
                            style="width: 50%;min-height: 140px;height: auto;background: white; box-shadow: 2px 4px 4px rgba(0, 0, 0, 0.429);border-radius: 4px;padding: 20px;margin-top: 20px;">
                            <div style="width: 100%;height: 70px;text-align: start;">
                                <span style="font-weight: 600;color: #49760f;margin-left: 11px;">为链接添加标识，可用于区分来源</span>
                                <div style="margin:20px 0px;">
                                    <input v-model="linkText" @input="addLogo(linkText)" placeholder="links"
                                        type="search" class="input">
                                </div>
                                <el-button type="success" @click="getQRcode()"><i class="fa fa-qrcode"></i>
                                    重新生成二维码</el-button>
                            </div>
                        </div>

                        <div
                            style="width: 50%;min-height: 210px;height: auto;background: white; box-shadow: 2px 4px 4px rgba(0, 0, 0, 0.429);border-radius: 4px;padding: 20px;margin-top: 20px;">
                            <div style="width: 100%;height: 70px;text-align: start;">
                                <span style="font-weight: 600;color: #49760f;margin-left: 11px;">分享到平台</span>
                                <div style="margin-top: 20px;">
                                    <svg t="1694749907473" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="9236" width="48" height="48">
                                        <path
                                            d="M704 397.92c-15.04-140.96-151.04-251.36-316.48-251.36-176 0-317.92 124.16-317.92 277.28a267.36 267.36 0 0 0 129.12 224c8.8 5.6-29.12 85.28-19.68 90.08s28.48-11.2 48.8-26.56 39.52-29.76 48-26.88a362.56 362.56 0 0 0 112 17.44 376.16 376.16 0 0 0 57.44-4.48c36.96 84.8 133.44 145.44 246.56 145.44a305.12 305.12 0 0 0 88.16-12.96c4.48-1.28 21.76 11.36 39.2 24.16s35.36 25.76 39.68 24c13.76-5.76-25.44-69.92-12.96-77.44 65.76-40.48 108.48-105.92 108.48-180-0.16-120-111.04-217.12-250.4-222.72z m-109.12 167.2a28 28 0 1 1 27.68-28 27.84 27.84 0 0 1-27.68 28z m-165.76 54.72a204.64 204.64 0 0 0 1.44 24.32 314.72 314.72 0 0 1-42.88 2.88 302.08 302.08 0 0 1-103.36-17.76c-3.2-1.12-14.4-4.64-20.48 0a265.28 265.28 0 0 0-32 32 142.4 142.4 0 0 0 8.96-38.4c1.12-10.24-14.56-17.6-17.76-19.68a220 220 0 0 1-98.08-178.4c0-122.88 117.6-222.4 262.72-222.4 135.36 0 246.88 86.72 260.96 198.24-124.48 17.44-219.52 108.96-219.52 219.2z m331.68-54.72a28 28 0 1 1 27.68-28 27.68 27.68 0 0 1-27.68 28z"
                                            fill="#8DC81B" p-id="9237"></path>
                                        <path
                                            d="M498.24 286.08a41.92 41.92 0 1 0 41.44 41.92 41.76 41.76 0 0 0-41.44-41.92zM276.96 286.08a41.92 41.92 0 1 0 41.6 41.92 41.6 41.6 0 0 0-41.6-41.92z"
                                            fill="#8DC81B" p-id="9238"></path>
                                    </svg>
                                    <svg t="1694750169983" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="21536" width="48" height="48">
                                        <path
                                            d="M512 954.24A442.24 442.24 0 1 0 69.76 512 442.08 442.08 0 0 0 512 954.24z m0-30.88a401.12 401.12 0 0 1-137.12-21.92V621.6l274.24 276.64A356 356 0 0 1 512 923.36z m285.28-119.68a400 400 0 0 1-112 81.28L487.2 687.04l389.44 1.92a359.52 359.52 0 0 1-79.2 114.72z m118.24-289.28a400 400 0 0 1-21.92 136.96H613.76l276.8-273.92a355.04 355.04 0 0 1 25.12 136.96z m-232.8-368a355.68 355.68 0 0 1 114.56 79.04 402.88 402.88 0 0 1 81.44 112L680.96 535.52zM512 653.6A141.6 141.6 0 1 1 653.6 512 141.6 141.6 0 0 1 512 653.6z m0-548.32A400 400 0 0 1 649.12 128v280L375.04 130.4A356.32 356.32 0 0 1 512 105.28z m-285.28 119.84a405.44 405.44 0 0 1 112-81.44l198.4 198.08-389.44-2.08a355.68 355.68 0 0 1 79.04-114.56zM108.64 514.4a400 400 0 0 1 21.92-136.96h279.84L133.6 651.36a357.92 357.92 0 0 1-24.96-136.96z m234.72-21.12l-1.92 389.44a357.12 357.12 0 0 1-114.72-79.04 401.76 401.76 0 0 1-81.28-112z"
                                            fill="#FFFFFF" p-id="21537"></path>
                                        <path
                                            d="M649.12 128A400 400 0 0 0 512 105.28a356.32 356.32 0 0 0-137.12 25.12l274.08 276.8z"
                                            fill="#FC6B4F" p-id="21538"></path>
                                        <path
                                            d="M797.44 225.12a355.68 355.68 0 0 0-114.56-79.04l-1.92 389.44 197.92-198.08a402.88 402.88 0 0 0-81.44-112.32z"
                                            fill="#7838F2" p-id="21539"></path>
                                        <path
                                            d="M893.76 651.36a400 400 0 0 0 21.92-136.96 355.04 355.04 0 0 0-25.12-136.96l-276.8 273.92z"
                                            fill="#5698F3" p-id="21540"></path>
                                        <path
                                            d="M685.12 884.96a400 400 0 0 0 112-81.28 359.52 359.52 0 0 0 79.2-114.72l-389.44-1.92z"
                                            fill="#20E9F4" p-id="21541"></path>
                                        <path
                                            d="M375.04 901.44A401.12 401.12 0 0 0 512 923.36a356 356 0 0 0 136.96-25.12L375.04 621.6z"
                                            fill="#00FD60" p-id="21542"></path>
                                        <path
                                            d="M341.44 882.72l1.92-389.44L145.44 691.2a401.76 401.76 0 0 0 81.28 112 357.12 357.12 0 0 0 114.72 79.52z"
                                            fill="#ABFB5B" p-id="21543"></path>
                                        <path
                                            d="M130.56 377.44a400 400 0 0 0-21.92 136.96 357.92 357.92 0 0 0 24.96 136.96l276.8-273.92z"
                                            fill="#F0E254" p-id="21544"></path>
                                        <path
                                            d="M339.04 144a405.44 405.44 0 0 0-112 81.44 355.68 355.68 0 0 0-79.04 114.56l389.44 2.08z"
                                            fill="#F6B351" p-id="21545"></path>
                                    </svg>
                                    <svg t="1694749854276" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="7255" width="48" height="48">
                                        <path
                                            d="M851.4 590.193c-22.196-66.233-90.385-90.422-105.912-91.863-15.523-1.442-29.593-9.94-19.295-27.505 10.302-17.566 29.304-68.684-7.248-104.681-36.564-36.14-116.512-22.462-173.094 0.866-56.434 23.327-53.39 7.055-51.65-8.925 1.89-16.848 32.355-111.02-60.791-122.395C311.395 220.86 154.85 370.754 99.572 457.15 16 587.607 29.208 675.873 29.208 675.873h0.58c10.009 121.819 190.787 218.869 412.328 218.869 190.5 0 350.961-71.853 398.402-169.478 0 0 0.143-0.433 0.575-1.156 4.938-10.506 8.71-21.168 11.035-32.254 6.668-26.205 11.755-64.215-0.728-101.66z m-436.7 251.27c-157.71 0-285.674-84.095-285.674-187.768 0-103.671 127.82-187.76 285.674-187.76 157.705 0 285.673 84.089 285.673 187.76 0 103.815-127.968 187.768-285.673 187.768z"
                                            fill="#E71F19" p-id="7256"></path>
                                        <path
                                            d="M803.096 425.327c2.896 1.298 5.945 1.869 8.994 1.869 8.993 0 17.7-5.328 21.323-14.112 5.95-13.964 8.993-28.793 8.993-44.205 0-62.488-51.208-113.321-114.181-113.321-15.379 0-30.32 3.022-44.396 8.926-11.755 4.896-17.263 18.432-12.335 30.24 4.933 11.662 18.572 17.134 30.465 12.238 8.419-3.46 17.268-5.33 26.41-5.33 37.431 0 67.752 30.241 67.752 67.247 0 9.068-1.735 17.857-5.369 26.202a22.832 22.832 0 0 0 12.335 30.236l0.01 0.01z"
                                            fill="#F5AA15" p-id="7257"></path>
                                        <path
                                            d="M726.922 114.157c-25.969 0-51.65 3.744-76.315 10.942-18.423 5.472-28.868 24.622-23.5 42.91 5.509 18.29 24.804 28.657 43.237 23.329a201.888 201.888 0 0 1 56.578-8.064c109.253 0 198.189 88.271 198.189 196.696 0 19.436-2.905 38.729-8.419 57.16-5.508 18.289 4.79 37.588 23.212 43.053 3.342 1.014 6.817 1.442 10.159 1.442 14.943 0 28.725-9.648 33.37-24.48 7.547-24.906 11.462-50.826 11.462-77.175-0.143-146.588-120.278-265.813-267.973-265.813z"
                                            fill="#F5AA15" p-id="7258"></path>
                                        <path
                                            d="M388.294 534.47c-84.151 0-152.34 59.178-152.34 132.334 0 73.141 68.189 132.328 152.34 132.328 84.148 0 152.337-59.182 152.337-132.328 0-73.15-68.19-132.334-152.337-132.334zM338.53 752.763c-29.454 0-53.39-23.755-53.39-52.987 0-29.228 23.941-52.989 53.39-52.989 29.453 0 53.39 23.76 53.39 52.989 0 29.227-23.937 52.987-53.39 52.987z m99.82-95.465c-6.382 11.086-19.296 15.696-28.726 10.219-9.43-5.323-11.75-18.717-5.37-29.803 6.386-11.09 19.297-15.7 28.725-10.224 9.43 5.472 11.755 18.864 5.37 29.808z"
                                            fill="#040000" p-id="7259"></path>
                                    </svg>
                                    <svg t="1694749977114" style="margin: 0 10px;" viewBox="0 0 1129 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="14260" width="48" height="48">
                                        <path
                                            d="M234.909 9.656a80.468 80.468 0 0 1 68.398 0 167.374 167.374 0 0 1 41.843 30.578l160.937 140.82h115.07l160.936-140.82a168.983 168.983 0 0 1 41.843-30.578A80.468 80.468 0 0 1 930.96 76.445a80.468 80.468 0 0 1-17.703 53.914 449.818 449.818 0 0 1-35.406 32.187 232.553 232.553 0 0 1-22.531 18.508h100.585a170.593 170.593 0 0 1 118.289 53.109 171.397 171.397 0 0 1 53.914 118.288v462.693a325.897 325.897 0 0 1-4.024 70.007 178.64 178.64 0 0 1-80.468 112.656 173.007 173.007 0 0 1-92.539 25.75h-738.7a341.186 341.186 0 0 1-72.421-4.024A177.835 177.835 0 0 1 28.91 939.065a172.202 172.202 0 0 1-27.36-92.539V388.662a360.498 360.498 0 0 1 0-66.789A177.03 177.03 0 0 1 162.487 178.64h105.414c-16.899-12.07-31.383-26.555-46.672-39.43a80.468 80.468 0 0 1-25.75-65.984 80.468 80.468 0 0 1 39.43-63.57M216.4 321.873a80.468 80.468 0 0 0-63.57 57.937 108.632 108.632 0 0 0 0 30.578v380.615a80.468 80.468 0 0 0 55.523 80.469 106.218 106.218 0 0 0 34.601 5.632h654.208a80.468 80.468 0 0 0 76.444-47.476 112.656 112.656 0 0 0 8.047-53.109v-354.06a135.187 135.187 0 0 0 0-38.625 80.468 80.468 0 0 0-52.304-54.719 129.554 129.554 0 0 0-49.89-7.242H254.22a268.764 268.764 0 0 0-37.82 0z m0 0"
                                            fill="#20B0E3" p-id="14261"></path>
                                        <path
                                            d="M348.369 447.404a80.468 80.468 0 0 1 55.523 18.507 80.468 80.468 0 0 1 28.164 59.547v80.468a80.468 80.468 0 0 1-16.094 51.5 80.468 80.468 0 0 1-131.968-9.656 104.609 104.609 0 0 1-10.46-54.719v-80.468a80.468 80.468 0 0 1 70.007-67.593z m416.02 0a80.468 80.468 0 0 1 86.102 75.64v80.468a94.148 94.148 0 0 1-12.07 53.11 80.468 80.468 0 0 1-132.773 0 95.757 95.757 0 0 1-12.875-57.133V519.02a80.468 80.468 0 0 1 70.007-70.812z m0 0"
                                            fill="#20B0E3" p-id="14262"></path>
                                    </svg>
                                    <svg t="1694750216360" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="23368" width="48" height="48">
                                        <path
                                            d="M955.728 428.224c8.385-8.785 3.76-23.536-8.073-25.753l-276.832-51.854c-4.838-0.906-9.02-3.987-11.38-8.383L525.873 93.229c-2.798-5.23-8.342-7.85-13.875-7.896-5.532 0.045-11.075 2.667-13.873 7.896L364.558 342.234c-2.36 4.396-6.543 7.477-11.381 8.383L76.345 402.471c-11.833 2.217-16.458 16.968-8.073 25.753L269.64 639.086c3.564 3.733 5.205 8.952 4.433 14.1l-46.015 282.032c-1.819 12.126 10.394 21.407 21.298 16.182L505 827.827a16.098 16.098 0 0 1 7-1.58 16.1 16.1 0 0 1 7.003 1.58L774.644 951.4c10.904 5.225 23.117-4.056 21.298-16.182l-46.88-287.298 206.666-219.696z"
                                            fill="#FFCD00" p-id="23369"></path>
                                        <path
                                            d="M559.42 493.63c-4.517-3.772-110.987-40.332-273.968-16-3.16 0.47-5.913-0.394-8.04-1.992-0.717 4 3.587 7.152 8.988 7.527 115.064 8.021 179.42 54.987 199.492 71.501 40.78-28.923 71.882-50.606 73.063-51.527 3.668-2.856 3.695-6.811 0.465-9.51m135.65-29.972c-41.744 35.168-160.159 116.897-201.52 148.468-4.864 3.711-3.177 9.424 2.098 11.43 17.045 6.488 36.23 11.95 56.421 16.445l159.784-152.228c12.544-13.184 5.238-29.152-10.422-32.661-1.025 3.011-3.259 5.933-6.36 8.546M817.187 640l-0.101 0.045c-70.456 29.709-241.54 79.623-451.762 72.33-25.386-0.88-50.63-3.715-64.786-6.325-2.067-0.38-3.878-1.012-5.476-1.846-10.567 12.224 2.073 21.462 47.148 30.58 131.886 26.676 286.047 38.934 415.304 30.665l-8.884-54.324c43.727-31.431 64.996-58.546 67.524-62.57 2.899-4.616 1.033-8.555 1.033-8.555"
                                            fill="#F1A308" p-id="23370"></path>
                                        <path
                                            d="M818.863 646.995c-53.31 5.137-215.894 3.686-311.826-33.167-5.107-1.962-6.834-7.566-2.129-11.194 40.025-30.84 154.62-110.68 195.014-145.035 7.872-6.696 9.95-15.437 0.375-22.542-18.36-13.623-83.168-36.203-158.198-36.816-107.373-0.88-212.858 29.498-259.133 54.09-10.983 5.837-4.392 21.221 6.83 19.495 164.223-25.24 271.495 12.756 276.045 16.67 3.255 2.798 3.074 6.906-0.5 9.715-3.036 2.389-199.263 143.36-258.23 193.11-9.286 7.834-6.845 24.246 8.35 27.018 14.152 2.582 39.406 5.412 64.784 6.284 210.173 7.214 381.314-42.24 451.755-71.63 0 0-2.148-7.057-13.137-5.998"
                                            fill="#FFFFFF" p-id="23371"></path>
                                    </svg>
                                    <svg t="1694750012492" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="19399" width="48" height="48">
                                        <path
                                            d="M511.09761 957.257c-80.159 0-153.737-25.019-201.11-62.386-24.057 6.702-54.831 17.489-74.252 30.864-16.617 11.439-14.546 23.106-11.55 27.816 13.15 20.689 225.583 13.211 286.912 6.767v-3.061z"
                                            fill="#FAAD08" p-id="19400"></path>
                                        <path
                                            d="M496.65061 957.257c80.157 0 153.737-25.019 201.11-62.386 24.057 6.702 54.83 17.489 74.253 30.864 16.616 11.439 14.543 23.106 11.55 27.816-13.15 20.689-225.584 13.211-286.914 6.767v-3.061z"
                                            fill="#FAAD08" p-id="19401"></path>
                                        <path
                                            d="M497.12861 474.524c131.934-0.876 237.669-25.783 273.497-35.34 8.541-2.28 13.11-6.364 13.11-6.364 0.03-1.172 0.542-20.952 0.542-31.155C784.27761 229.833 701.12561 57.173 496.64061 57.162 292.15661 57.173 209.00061 229.832 209.00061 401.665c0 10.203 0.516 29.983 0.547 31.155 0 0 3.717 3.821 10.529 5.67 33.078 8.98 140.803 35.139 276.08 36.034h0.972z"
                                            fill="#000000" p-id="19402"></path>
                                        <path
                                            d="M860.28261 619.782c-8.12-26.086-19.204-56.506-30.427-85.72 0 0-6.456-0.795-9.718 0.148-100.71 29.205-222.773 47.818-315.792 46.695h-0.962C410.88561 582.017 289.65061 563.617 189.27961 534.698 185.44461 533.595 177.87261 534.063 177.87261 534.063 166.64961 563.276 155.56661 593.696 147.44761 619.782 108.72961 744.168 121.27261 795.644 130.82461 796.798c20.496 2.474 79.78-93.637 79.78-93.637 0 97.66 88.324 247.617 290.576 248.996a718.01 718.01 0 0 1 5.367 0C708.80161 950.778 797.12261 800.822 797.12261 703.162c0 0 59.284 96.111 79.783 93.637 9.55-1.154 22.093-52.63-16.623-177.017"
                                            fill="#000000" p-id="19403"></path>
                                        <path
                                            d="M434.38261 316.917c-27.9 1.24-51.745-30.106-53.24-69.956-1.518-39.877 19.858-73.207 47.764-74.454 27.875-1.224 51.703 30.109 53.218 69.974 1.527 39.877-19.853 73.2-47.742 74.436m206.67-69.956c-1.494 39.85-25.34 71.194-53.24 69.956-27.888-1.238-49.269-34.559-47.742-74.435 1.513-39.868 25.341-71.201 53.216-69.974 27.909 1.247 49.285 34.576 47.767 74.453"
                                            fill="#FFFFFF" p-id="19404"></path>
                                        <path
                                            d="M683.94261 368.627c-7.323-17.609-81.062-37.227-172.353-37.227h-0.98c-91.29 0-165.031 19.618-172.352 37.227a6.244 6.244 0 0 0-0.535 2.505c0 1.269 0.393 2.414 1.006 3.386 6.168 9.765 88.054 58.018 171.882 58.018h0.98c83.827 0 165.71-48.25 171.881-58.016a6.352 6.352 0 0 0 1.002-3.395c0-0.897-0.2-1.736-0.531-2.498"
                                            fill="#FAAD08" p-id="19405"></path>
                                        <path
                                            d="M467.63161 256.377c1.26 15.886-7.377 30-19.266 31.542-11.907 1.544-22.569-10.083-23.836-25.978-1.243-15.895 7.381-30.008 19.25-31.538 11.927-1.549 22.607 10.088 23.852 25.974m73.097 7.935c2.533-4.118 19.827-25.77 55.62-17.886 9.401 2.07 13.75 5.116 14.668 6.316 1.355 1.77 1.726 4.29 0.352 7.684-2.722 6.725-8.338 6.542-11.454 5.226-2.01-0.85-26.94-15.889-49.905 6.553-1.579 1.545-4.405 2.074-7.085 0.242-2.678-1.834-3.786-5.553-2.196-8.135"
                                            fill="#000000" p-id="19406"></path>
                                        <path
                                            d="M504.33261 584.495h-0.967c-63.568 0.752-140.646-7.504-215.286-21.92-6.391 36.262-10.25 81.838-6.936 136.196 8.37 137.384 91.62 223.736 220.118 224.996H506.48461c128.498-1.26 211.748-87.612 220.12-224.996 3.314-54.362-0.547-99.938-6.94-136.203-74.654 14.423-151.745 22.684-215.332 21.927"
                                            fill="#FFFFFF" p-id="19407"></path>
                                        <path
                                            d="M323.27461 577.016v137.468s64.957 12.705 130.031 3.91V591.59c-41.225-2.262-85.688-7.304-130.031-14.574"
                                            fill="#EB1C26" p-id="19408"></path>
                                        <path
                                            d="M788.09761 432.536s-121.98 40.387-283.743 41.539h-0.962c-161.497-1.147-283.328-41.401-283.744-41.539l-40.854 106.952c102.186 32.31 228.837 53.135 324.598 51.926l0.96-0.002c95.768 1.216 222.4-19.61 324.6-51.924l-40.855-106.952z"
                                            fill="#EB1C26" p-id="19409"></path>
                                    </svg>
                                    <svg t="1694750081159" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="20436" width="48" height="48">
                                        <path
                                            d="M1024 862.826667a161.152 161.152 0 0 1-161.173333 161.173333h-701.632a161.152 161.152 0 0 1-161.173334-161.173333v-701.632a161.194667 161.194667 0 0 1 161.173334-161.173334h701.632a161.194667 161.194667 0 0 1 161.173333 161.173334v701.632z"
                                            fill="#306FB6" p-id="20437"></path>
                                        <path
                                            d="M828.842667 250.261333h-173.461334v-36.522666a17.066667 17.066667 0 0 0-17.066666-17.066667h-41.941334a17.045333 17.045333 0 0 0-17.045333 17.066667v263.296h-24.896a17.066667 17.066667 0 0 0-17.045333 17.066666V810.218667a17.066667 17.066667 0 0 0 17.045333 17.088h257.365333a17.066667 17.066667 0 0 0 17.066667-17.088v-58.986667h-0.256v-198.144h0.256v-58.986667a17.066667 17.066667 0 0 0-17.066667-17.066666H655.381333v-150.72h173.461334a17.066667 17.066667 0 0 0 17.066666-17.066667v-41.941333a17.066667 17.066667 0 0 0-17.066666-17.045334z m-93.376 302.848c9.429333 0 17.045333 7.637333 17.045333 17.066667v164.010667a17.066667 17.066667 0 0 1-17.045333 17.066666h-104.981334a17.045333 17.045333 0 0 1-17.045333-17.066666v-164.010667a17.066667 17.066667 0 0 1 17.045333-17.066667h104.981334zM197.909333 657.408h41.941334a17.088 17.088 0 0 0 17.088-17.088v-358.741333h174.528v358.741333a17.066667 17.066667 0 0 0 17.045333 17.088h41.941333a17.088 17.088 0 0 0 17.088-17.088v-417.749333c0-9.408-7.658667-17.066667-17.088-17.066667h-292.522666a17.066667 17.066667 0 0 0-17.045334 17.066667v417.749333a17.024 17.024 0 0 0 17.024 17.088z"
                                            fill="#FFFFFF" p-id="20438"></path>
                                        <path
                                            d="M492.970667 739.2v-0.170667a110.656 110.656 0 0 1-110.656-110.656h-0.106667v-296.042666a17.066667 17.066667 0 0 0-17.066667-17.045334h-41.941333a17.045333 17.045333 0 0 0-17.045333 17.045334v296.042666h-0.362667a110.677333 110.677333 0 0 1-110.677333 110.656v0.170667a17.045333 17.045333 0 0 0-17.045334 17.066667v42.112a17.066667 17.066667 0 0 0 17.045334 17.066666 186.602667 186.602667 0 0 0 148.928-74.176 186.624 186.624 0 0 0 148.928 74.176 17.045333 17.045333 0 0 0 17.045333-17.066666v-42.112a17.024 17.024 0 0 0-17.045333-17.066667z"
                                            fill="#FFFFFF" p-id="20439"></path>
                                        <path
                                            d="M865.450667 873.685333c17.664-3.797333 15.253333-24.896 14.741333-29.504-0.874667-7.125333-9.237333-19.541333-20.586667-18.581333-14.293333 1.28-16.362667 21.930667-16.362666 21.930667-1.962667 9.557333 4.608 29.952 22.208 26.154666z m18.752 36.693334c-0.533333 1.493333-1.664 5.290667-0.682667 8.597333 1.962667 7.402667 8.405333 7.744 8.405333 7.744h9.258667v-22.613333h-9.898667a10.837333 10.837333 0 0 0-7.082666 6.272z m14.037333-72.085334c9.749333 0 17.621333-11.221333 17.621333-25.109333 0-13.866667-7.872-25.088-17.621333-25.088-9.728 0-17.642667 11.221333-17.642667 25.088 0 13.888 7.893333 25.109333 17.642667 25.109333z m41.984 1.664c13.034667 1.706667 21.418667-12.202667 23.082667-22.762666 1.706667-10.538667-6.698667-22.762667-15.936-24.853334-9.258667-2.112-20.778667 12.672-21.845334 22.357334-1.258667 11.797333 1.685333 23.573333 14.698667 25.258666z m31.936 61.973334s-20.16-15.594667-31.936-32.448c-15.957333-24.874667-38.634667-14.762667-46.208-2.112-7.552 12.650667-19.328 20.629333-20.992 22.741333-1.685333 2.090667-24.362667 14.336-19.328 36.672 5.034667 22.336 22.72 21.909333 22.72 21.909333s13.013333 1.28 28.138667-2.112c15.104-3.349333 28.117333 0.832 28.117333 0.832s35.306667 11.818667 44.970667-10.944c9.621333-22.72-5.482667-34.538667-5.482667-34.538666z m-60.394667 33.877333h-22.954666c-9.92-1.984-13.866667-8.746667-14.357334-9.898667-0.490667-1.173333-3.306667-6.613333-1.813333-15.850666 4.288-13.866667 16.490667-14.848 16.490667-14.848h12.224v-15.04l10.410666 0.149333v55.488z m42.752-0.170667h-26.410666c-10.24-2.624-10.709333-9.92-10.709334-9.92v-29.205333l10.709334-0.170667v26.261334c0.64 2.794667 4.117333 3.306667 4.117333 3.306666h10.88v-29.397333h11.413333v39.125333z m37.376-77.994666c0-5.034667-4.181333-20.224-19.733333-20.224-15.552 0-17.642667 14.336-17.642667 24.469333 0 9.685333 0.810667 23.189333 20.16 22.741333 19.328-0.426667 17.216-21.909333 17.216-26.986666z"
                                            fill="#FFFFFF" p-id="20440"></path>
                                    </svg>
                                    <svg t="1694750324343" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="24518" width="48" height="48">
                                        <path
                                            d="M0 0m256 0l512 0q256 0 256 256l0 512q0 256-256 256l-512 0q-256 0-256-256l0-512q0-256 256-256Z"
                                            fill="#3297F9" p-id="24519"></path>
                                        <path
                                            d="M802.56 431.744c-1.28 5.376-4.48 13.312-8.96 22.784h0.128l-0.512 0.896c-25.984 55.168-93.568 163.456-93.568 163.456l-0.384-0.64-19.84 34.304h95.36L592.768 893.44l41.344-163.84H559.104l26.112-108.416c-21.12 4.992-45.952 12.032-75.52 21.504 0 0-39.936 23.296-115.072-44.8 0 0-50.688-44.416-21.248-55.552 12.544-4.736 60.672-10.752 98.56-15.744 51.2-6.912 82.688-10.496 82.688-10.496S396.8 518.4 359.296 512.64c-37.504-5.888-84.992-67.968-95.104-122.624 0 0-15.616-29.952 33.664-15.744 49.28 14.208 253.312 55.296 253.312 55.296S285.696 348.544 268.032 328.832c-17.664-19.712-51.968-107.776-47.488-161.92 0 0 1.92-13.44 15.872-9.856 0 0 196.224 89.216 330.368 138.112 134.144 48.512 250.752 73.344 235.776 136.576z"
                                            fill="#FFFFFF" p-id="24520"></path>
                                    </svg>
                                    <svg t="1694750379016" style="margin: 0 10px;" viewBox="0 0 1025 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="25490" width="48" height="48">
                                        <path
                                            d="M321.828571 928.212916c386.354599 0 597.767515-320.22544 597.767515-597.767515 0-9.017613-0.200391-18.235616-0.601174-27.253229C960.075147 273.534247 995.744814 236.46184 1023.799609 194.379648c-37.673581 16.832877-78.152642 28.054795-120.635617 33.064579 43.28454-26.050881 76.749902-67.131115 92.380431-116.227006-40.679452 24.046967-85.567123 41.481018-133.460666 50.899413-38.274755-40.879843-92.981605-66.32955-153.299413-66.32955-116.026614 0-210.010176 94.183953-210.010176 210.010176 0 16.432094 1.803523 32.463405 5.410568 47.893542C329.643836 344.873581 174.741292 261.310372 71.138943 134.262231c-18.035225 31.060665-28.455577 67.131115-28.455577 105.606262 0 72.942466 37.072407 137.268102 93.582779 174.941683-34.467319-1.001957-66.930724-10.620744-95.18591-26.251272v2.605088c0 101.798826 72.341292 186.764775 168.529158 206.002349-17.634442 4.809393-36.270841 7.414481-55.308023 7.414481-13.426223 0-26.652055-1.40274-39.477104-3.807436 26.652055 83.563209 104.403914 144.2818 196.18317 145.884931-71.940509 56.30998-162.517417 89.975734-260.909589 89.975734-17.033268 0-33.665753-1.001957-50.097847-3.005871 92.781213 59.716634 203.196869 94.584736 321.828571 94.584736"
                                            fill="#1DA1F2" p-id="25491"></path>
                                    </svg>
                                    <svg t="1694750407563" style="margin: 0 10px;" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="26463" width="48" height="48">
                                        <path
                                            d="M880 112H144c-17.7 0-32 14.3-32 32v736c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V144c0-17.7-14.3-32-32-32z m-92.4 233.5h-63.9c-50.1 0-59.8 23.8-59.8 58.8v77.1h119.6l-15.6 120.7h-104V912H539.2V602.2H434.9V481.4h104.3v-89c0-103.3 63.1-159.6 155.3-159.6 44.2 0 82.1 3.3 93.2 4.8v107.9z"
                                            fill="#1296DB" p-id="26464"></path>
                                    </svg>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </el-col>
        </el-row>
    </div>
</template>

<script>
import QRCode from 'qrcode'
import LeftBody from '@/components/formDetails/LeftBody.vue'

export default {
    components: {
        LeftBody
    },
    data() {
        return {
            key: this.$route.query.key,
            linkDataOnLine: '',
            linkDataLocal: 'http://localhost:8080/build?key=' + this.$route.query.key,
            citys: '',
            isChecked: false,
            QRImgUrl: '',
            baseUrl: window.location.origin + '/build?key=',
            isDetails: false,
            linkText: ''
        }
    },
    created() {
        this.linkDataOnLine = this.baseUrl + this.$route.query.key
        this.getQRcode()
        if (this.$route.query.type && this.$route.query.type === 'details') {
            this.isDetails = true
        }
    },
    methods: {
        saveImage() {
            // 创建一个虚拟链接
            const link = document.createElement('a');
            link.href = this.QRImgUrl;
            link.download = 'image.jpg'; // 下载的文件名

            // 模拟点击下载链接
            document.body.appendChild(link);
            link.click();

            // 移除虚拟链接
            document.body.removeChild(link);
        },
        async getQRcode() {
            let opts = {
                errorCorrectionLevel: "L", // 容错级别
                type: "image/png", // 生成的二维码类型
                quality: 0.3, // 二维码质量
                margin: 5, // 二维码留白边距
                width: 128, // 宽
                height: 128, // 高
                text: "http://www.xxx.com", // 二维码内容，确保this.QRlink包含有效的URL
                color: {
                    dark: "#3f571f", // 前景色
                    light: "#fff" // 背景色
                }
            };

            try {
                // 使用async/await来等待QRCode.toDataURL的结果
                this.QRImgUrl = await QRCode.toDataURL(this.linkDataOnLine, opts)
                // console.log(this.QRImgUrl)
            } catch (err) {
                console.error(err);
            }
        },
        copyLink() {
            var context = this.linkDataOnLine
            const el = document.createElement('textarea')
            el.value = context
            document.body.appendChild(el)
            el.select()
            document.execCommand('copy')
            document.body.removeChild(el)
            this.$message.success('复制成功：' + context)
        },
        openInNewTab() {
            // 在新选项卡中打开一个URL
            window.open(this.linkDataOnLine, '_blank')
        },
        openInNewTabTest() {
            // 在新选项卡中打开一个URL
            window.open(this.linkDataLocal, '_blank')
        },
        goback() {
            this.$router.push({ path: '/formDetails', query: { key: this.$route.query.key } })
        },
        gobackDetails() {
            this.$router.push({ path: '/form/details', query: { key: this.$route.query.key } })
        },
        changeCitys() {
            this.linkDataOnLine = 'https://tduck-vue.foxconnedu.com/s/a7378a8b499b4fbc923334bb0683c518?factoryarea=' + this.citys
        },
        addLogo(text) {
            this.linkDataOnLine = this.baseUrl + this.$route.query.key
            if (text) {
                this.linkDataOnLine = this.linkDataOnLine + '?source=' + text
            }
        }
    }
}
</script>

<style scoped>
.MenuBtn {
    cursor: pointer;
    height: 40px;
    line-height: 40px;
    font-size: 20px;
    font-weight: 500;
    text-align: start;
    padding-left: 29px;
}

.MenuBtn:hover {
    /* background-color: #d3cfdd79; */
    background-color: #0f35546a;
    border-radius: 5px;
}

.group {
    display: flex;
    line-height: 28px;
    align-items: center;
    position: relative;
    max-width: 1200px;
    margin-top: 10px;
}

.input {
    width: 100%;
    height: 40px;
    line-height: 28px;
    padding: 0 1rem;
    padding-left: 2.5rem;
    border: 2px solid transparent;
    border-radius: 8px;
    outline: none;
    background-color: #f3f3f4;
    color: #49760f;
    transition: .3s ease;
}

.input::placeholder {
    color: #a7ce74;
}

.input:focus,
input:hover {
    outline: none;
    border-color: rgba(126, 234, 76, 0.4);
    background-color: #fff;
    box-shadow: 0 0 0 4px rgba(145, 234, 76, 0.1);
}

.icon {
    position: absolute;
    left: 11px;
    fill: #9e9ea7;
    width: 32px;
    height: 32px;
}
</style>